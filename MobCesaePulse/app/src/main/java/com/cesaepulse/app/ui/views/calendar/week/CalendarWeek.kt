...
fun CalendarWeek(
			...
			Card(
				modifier = Modifier
					...
					.clickable { // Adiciona a ação de clique
						viewModel.changeSelectedCard(i)// Definir o número do card selecionado
						viewModel.changeOpenDialog() // Abre o Dialog
	...
	if (openDialog) {
		AlertDialog(onDismissRequest = {
			...
			title = {
				Text(text = "${schedulers[selectedCard]?.created_at.toString().split(" ")[0].split('-')[2]} de $selectedText") // Título do Dialog
			}, text = {
				// Exibe o texto correspondente ao card selecionado
				CalendarDayDetails(cardTexts[selectedCard])// Texto diferente para cada card
			}, confirmButton = {
				TextButton(onClick = { viewModel.changeOpenDialog() }) {
					Text("Fechar")
				}
			})
	}

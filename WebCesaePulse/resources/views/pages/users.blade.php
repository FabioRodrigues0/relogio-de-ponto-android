...
@section('content')
    <div class="container mt-5">
        ...
        @if (session('eliminated'))
            <div id="danger-alert" class="alert alert-danger">{{ session('eliminated') }}</div>
        @endif
                                ...
                                <td>
                                    @if ($user->status == 'pending')
                                        <svg class="notification m-3" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                            width="30" height="30" viewBox="0 0 50 50" style="fill:#FAB005;">
                                            <path
        ...
        <div class="row mb-4 mt-4">
                ...
                <div class="card text-center shadow">
                    <div class="card-body">
                        ...
                        @if ($countUsersNumber > 0)
                            <p class="card-text fs-4" style="color: #1E0C3C)">{{ $countUsersNumber }}</p>
                        @else
            ...
            <div class="col-4">
                <div class="card text-center shadow">
                    <div class="card-body">
                        ...
                        @if ($countPasswordsNumber > 0)
                            <p class="card-text fs-4" style="color:red">{{ $countPasswordsNumber }}</p>
                        @else
            ...
            <div class="col-4">
                <div class="card text-center shadow">
                    <div class="card-body">
                        <h5 class="card-title">Último update</h5>
                        @if ($lastUserUpdate)
                            <p class="card-text fs-4">{{ $lastUserUpdate->updated_at }}</p>
                        @else
                            <p class="card-text fs-4">Sem registos.</p>
                        @endif
                    </div>
                </div>
            </div>
